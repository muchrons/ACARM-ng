...
TimeConverter::TimeConverter(void):
  the1900_(-2208988800ll)   // GNU epoch timestamp for 1990.01.01, midnight, UTC, generated by: date -d "1 Jan 1900 UTC" +%s
{
}

std::string TimeConverter::toNtpStamp(const Persistency::Timestamp &t) const
{
  const int64_t  zero =0;                                       // 0==invalid/undefined according to RFC-1305
  const int64_t  diff =std::max( t.get()-the1900_, zero );      // compute diff, or set to zero
  assert(diff>=0);
  const uint32_t stamp=(diff>0xFFFFFFFFu)?0:diff;               // out of range? only 32-bits are allowed.
  //assert(stamp>=0);
  //assert(stamp<=0xFFFFFFFFu);
  const uint32_t major=stamp;                                   // enusre proper range-check
...
TimeConverter::ExactTimestamp TimeConverter::fromNtpStamp(const std::string &str) const
{
  ...
  const int64_t sec =major+the1900_;            // convert to GNU epoch
