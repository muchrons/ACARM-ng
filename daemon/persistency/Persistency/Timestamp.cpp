...
std::string Timestamp::str(void) const
{
  ...
  if( gmtime_r(&tt, &bt)!=&bt )
    throw ExceptionConvertionError(SYSTEM_SAVE_LOCATION, "gmtime_r() returned invalid pointer");
  ...
  if( snprintf(tmp, sizeof(tmp), "%04d.%02d.%02d %02d:%02d:%02d",
    ...
    throw ExceptionConvertionError(SYSTEM_SAVE_LOCATION, "snprintf() wrote invalid number of characters");
