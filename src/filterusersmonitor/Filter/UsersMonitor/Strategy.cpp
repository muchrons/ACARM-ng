...
bool Strategy::isEntryInteresting(const NodeEntry thisEntry) const
{
  ...
  const Data d( thisEntry.node_->getAlert(), params_.skip_ );
  if(d.get().size()==0u)
    return false;
...
{
  const Data::Names::const_iterator it=thisEntry.t_.commonWith(otherEntry.t_);
...
bool Strategy::canCorrelate(const NodeEntry thisEntry, const NodeEntry otherEntry) const
{
