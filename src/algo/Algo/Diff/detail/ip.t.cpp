...
typedef Algo::Diff::detail::Comparer<const boost::asio::ip::address> Cmp;
...
{
  ensure_equals("identical IPs differ", Cmp::cmp( ip("1.2.3.4"), ip("1.2.3.4") ).get(), 1);
}

// test comparing totally different IPs
template<>
template<>
void testObj::test<2>(void)
{
  ensure("different IPs are too similar", Cmp::cmp( ip("250.250.250.250"), ip("1.1.1.1") ).get() < 0.01);
}
...
{
  const Similarity s=Cmp::cmp( ip("10.11.12.13"), ip("10.11.12.14") );
  ensure("value is too small", s.get()>0.75);
  ensure("IPs reported identical", s.get()<1);
}

// test comparing highly different IPs
template<>
template<>
...
{
  const Similarity s=Cmp::cmp( ip("10.11.12.13"), ip("::abcd") );
...
{
  const Similarity s=Cmp::cmp( ip("::1234:5678:aabb"), ip("::1234:5678:abcd") );
...
{
  const Similarity s1=Cmp::cmp( ip("10.11.12.13"), ip("10.11.12.12") );
  const Similarity s2=Cmp::cmp( ip("10.11.12.13"), ip("10.11.12.32") );
...
{
  const Similarity s=Cmp::cmp( ip("1234::1020:3040"), ip("ABCD::1020:3040") );
...
{
  const Similarity s=Cmp::cmp( ip("10.11.12.13"), ip("66.11.12.13") );
  ensure("similarity too high", s.get()<0.01);
}
